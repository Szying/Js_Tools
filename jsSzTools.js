...
(function(m) {
    ...
    m.util = {
        ...
        getMoneyClassFormat: function() {
            var isNum = $(".num").html();
            $(".num").html($ML.util.getMoneyFormat(isNum));
        },
        blockUI: function(settings) {
            var _settings = {
                ...
                ajaxStart: function() {
                    switch (_settings.showStyle) {
                        case 1:
                            $(_settings.progressbarDiv).html("<div id='progressbar' style='display:none;''><div class='progress-label' '></div></div>");
            ...
            if (!$ML.util.isEmptyStr(settings)) {
    ...
    m.auth = {
        valid: function(customerCallback) {
            ...
            $.each(tel, function(i, v) {
                var val = $(this).val();
                if (!$ML.util.checkNumber(val) || val.length < 5 ||  val.length > 8) {
            ...
            $.each(email, function(i, v) {
                var val = $(this).val();
                if (val.length < 8 || !$ML.util.checkEmail(val)) {
            ...
            $.each(phone, function(i, v) {
                var val = $(this).val();
                if (!$ML.util.checkNumber(val) || val.length != 10) {
                    //$(this).after("<span class='alertMsg'>格式錯誤，請全部填入數字</span>");
                	$(this).attr("data-container","body");
                	$(this).attr("data-toggle","popover");
                	var errorMsg=$(this).attr("data-content")==undefined?"":$(this).attr("data-content");
            ...
            $.each(num, function(i, v) {
                var val = $(this).val();
                if (!$ML.util.checkIntegerNumber(val) || parseInt(val) < 1) {
            ...
            $.each(telArea, function(i, v) {
                var val = $(this).val();
                if (!$ML.util.checkIntegerNumber(val) || val.length > 4 || val.length < 2) {
                    //$(this).after("<span class='alertMsg'>格式錯誤，請填入大於1整數</span>");
                	$(this).attr("data-container","body");
                	$(this).attr("data-toggle","popover");
                	var errorMsg=$(this).attr("data-content")==undefined?"":$(this).attr("data-content");
            ...
            $.each(compID, function(i, v) {
                var val = $(this).val();
                if (!$ML.util.checkIntegerNumber(val) || val.length != 8) {
                    //$(this).after("<span class='alertMsg'>格式錯誤，請填入大於1整數</span>");
                	$(this).attr("data-container","body");
                	$(this).attr("data-toggle","popover");
                	var errorMsg=$(this).attr("data-content")==undefined?"":$(this).attr("data-content");
